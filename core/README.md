...
- cached remote-shard ranking is now outcome-aware too: `shard_matcher.py` attaches measured reuse summaries to cached `peer_received` candidates, and `shard_ranker.py` gives only a bounded boost to remote shards that have actually improved clean final answers instead of rewarding incidental citation history or weaker answer-backed-only proof
