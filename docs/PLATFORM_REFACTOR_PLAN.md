...
| `storage/shard_reuse_outcomes.py` | 265 | downstream remote-shard reuse outcomes now persist here with selected-vs-answer-backed summaries, while turn reasoning now has to earn `answer_backed` via a no-shard counterfactual; keep the persistence/summary truth here instead of duplicating it in loaders or ranking seams |
