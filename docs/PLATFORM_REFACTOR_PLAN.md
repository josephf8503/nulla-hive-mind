...
| `core/orchestration/executor.py` | 663 | bounded local subtask executor now enforces capacity-blocked worker lanes, records scheduling details, resolves step-to-step runtime references for the bounded operator lane, and records verified procedure reuse metrics; keep it focused on local queen/coder/verifier execution and do not let it turn into a second mesh scheduler |
