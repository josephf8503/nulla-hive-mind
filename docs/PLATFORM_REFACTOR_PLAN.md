...
- keep the execution planner honest: explicit patch-and-validate repo requests should plan into bounded queen/coder/verifier envelopes instead of falling back to flat tool chains and calling that orchestration
...
| `core/orchestration/executor.py` | 435 | bounded local subtask executor now also enforces capacity-blocked worker lanes and records scheduling details; keep it focused on local queen/coder/verifier execution and do not let it turn into a second mesh scheduler |
