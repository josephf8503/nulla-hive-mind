...
- keep runtime provider bootstrap truth shared: default local Ollama and configured remote lanes like Kimi should register through one seam so backbone snapshots, doctor/install truth, and API bootstrap do not drift apart
...
| `core/orchestration/executor.py` | 687 | bounded local subtask executor now enforces capacity-blocked worker lanes, records scheduling details, resolves step-to-step runtime references for the bounded operator lane, supports validation-only allowed-failure preflight capture for bounded repair loops, records verified procedure reuse metrics, and now fails closed when child-merge truth says the final verifier still lost; keep it focused on local queen/coder/verifier execution and do not let it turn into a second mesh scheduler |
