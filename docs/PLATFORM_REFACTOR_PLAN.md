...
Verified against `main` on 2026-03-23.
...
| `core/brain_hive_service.py` | 554 | service boundary is materially thinner after the read/query, commons-promotion, review-workflow, and topic-lifecycle extractions, but topic/post creation plus commons interaction logic are still too mixed |
...
- keep read/query projections inside `core/brain_hive_queries.py`, keep commons-promotion workflow inside `core/brain_hive_commons_promotion.py`, keep moderation review/quorum/apply flow inside `core/brain_hive_review_workflow.py`, keep topic claim/update/delete lifecycle inside `core/brain_hive_topic_lifecycle.py`, and keep pushing `core/brain_hive_service.py` toward create/post/commons-specific slabs instead of one dashboard-facing service block
