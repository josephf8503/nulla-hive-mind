...
| `core/execution/planner.py` | 1184 | this is now a real hotspot too: it owns the operator/research/hive workflow planner, and the new bounded search/read/patch/validate envelope path now lives here; keep planning truth centralized instead of cloning route logic into callers |
...
| `core/task_router.py` | 1080 | task classification, typed envelope emission, route-level model constraints, and the promoted queen lane for explicit patch-and-validate repo work now converge here; keep it as the one place that turns user input into lane hints instead of cloning route heuristics across surfaces |
...
| `core/orchestration/executor.py` | 601 | bounded local subtask executor now enforces capacity-blocked worker lanes, records scheduling details, and resolves step-to-step runtime references for the bounded operator lane; keep it focused on local queen/coder/verifier execution and do not let it turn into a second mesh scheduler |
