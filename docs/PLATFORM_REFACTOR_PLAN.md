...
- Signed API presence writes and signed bootstrap snapshots now also feed that same proof-backed endpoint seam, and the meet/bootstrap contracts now carry `endpoints` lists while keeping the old `endpoint` field as a best-endpoint compatibility alias. That is still a compatibility bridge, not the final authoritative multi-endpoint store.
