...
- keep candidate-repair promotion narrow and honest: only synthesize a bounded repair envelope from diagnosis when the failing test expectation and unread implementation read make the literal fix explicit, or when a one-hop no-arg delegate helper read makes the same literal fix explicit, otherwise stay read-only and fail closed instead of hallucinating a patch plan
...
| `core/shard_ranker.py` | 98 | candidate scoring now gives bounded priority only to remote shards with answer-backed reuse proof; keep this as the one ranking truth instead of inventing separate Hive-reuse scoring inside loaders or chat surfaces |
| `core/tiered_context_loader.py` | 745 | context assembly is still a real hotspot; remote shard citations, measured reuse-outcome summaries, answer-backed proof notes, and ranked remote-cache ordering now flow through this loader, so keep it as assembly logic instead of growing a second receipt/transport implementation inside it |
| `storage/shard_reuse_outcomes.py` | 265 | downstream remote-shard reuse outcomes now persist here with selected-vs-answer-backed summaries; keep attribution logic out of the loader and turn-reasoning seams so this stays the one place that records Hive-reuse results |
