...
- `core/execution/planner.py`: workflow planner for research/operator/hive flows, including the bounded search/read/patch/validate envelope path, raw fenced unified-diff repair planning, preflight failing-test repair planning for clear repo edit requests, validation-result followup planning into inspect/search steps that can continue past the first file read and into the first unread repo match, and rollback-on-failure verifier planning for bounded repairs
